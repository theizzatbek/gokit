...
### `ErrsvalBindError` — recommended bind-error handler
...
После этого все parse/validate failures из `bind.Body / Query / Params / Header` (и `RegisterHandlerWithInput`) маппятся в kit-овский `{code, message, details[]}` JSON:
