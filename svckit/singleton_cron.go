...
//
//	svckit.WithSingletonCron("daily-rollup", "0 3 * * *", rollup.Run)
//
// Lock semantics:
//
...
//
// Requires Config.DB to be set. Errors at New time otherwise with
// [CodeSingletonCronNeedsDB].
...
// [WithSingletonCron] — registers a singleton job AFTER New has
// finished building, so the job's closure can capture svc.DB /
// svc.Logger / etc.
func (s *Service[T, C]) AddSingletonCron(name, schedule string, fn JobFn) error {
	if s.DB == nil {
		return xerrs.Validation(CodeSingletonCronNeedsDB,
			"svckit: AddSingletonCron requires DB to be configured")
	}
