...
6. **CORS.** Для браузерных клиентов — `WithCORS("https://app.example.com")` или `WithCORSConfig(cors.Config{...})`. С v1.1.0 env-вариант: `CORS_ORIGINS=https://a.com,https://b.com` в env поднимет `WithCORS(origins...)` с kit-defaulted `cors.Config`, если caller не вызвал `WithCORS`/`WithCORSConfig` сам. Без CORS preflight'ы летят в 404 и ваш SPA молча отлуплен. CORS-слой ставится ДО авто-`Bearer(BearerOptional)`-слоя, поэтому 401 от протухшего/битого access-токена несёт `Access-Control-Allow-Origin` и браузерный флоу «поймал 401 → пошёл на refresh» работает; preflight OPTIONS коротит в CORS, не задевая auth.
...
10. **`SetBindErrorHandler` с `ErrsvalBindError`.** Kit default'ный bind-error handler пишет `{"error":"..."}` plain-text — fibermap-пакет не форсит errs-convention. Для production-сервисов, использующих `*errs.Error`-wire-shape повсеместно, передавайте `fibermap.ErrsvalBindError[T]` (с v1.1.0): `svc.Engine.SetBindErrorHandler(fibermap.ErrsvalBindError[AppCtx])`. Это даёт `{code, message, details[]}` с source-aware code'ами (`invalid_body` / `invalid_query` / `invalid_params` / `invalid_header`).
...
- **`WithValidator(v)` полностью заменяет default validator** — если хотите kit-defaults + один custom тэг, используйте `WithExtraValidators(map[string]validator.Func{"slug_chars": isSafeSlug})` (с v1.1.0). Tags регистрируются ПОВЕРХ default'ного `validator.New(validator.WithRequiredStructEnabled())`. `WithValidator` оставлен для callers'ов с полностью кастомной реализацией; при одновременной передаче `WithValidator` extras silent'но игнорируются (кит не мутирует caller-shared instance).
- **API-key middleware silent panic при пустом `APIKeyHashSecret`.** До v1.1.0 `service.AuthConfig` не имел поля для секрета — каждый сервис, wire'ивший API-key auth, либо обходил `service.WithAuth` (строя `auth.Auth[C]` руками), либо ловил `CodeAPIKeyMissingSecret` panic на первом запросе. С v1.1.0 секрет идёт через `AUTH_APIKEY_HASH_SECRET=<base64>` env (≥ 32 байт после decode) либо `cfg.Auth.APIKeyHashSecret` напрямую. Invalid → boot-time validation error, не runtime panic. Для callers'ов, строящих `auth.Auth[C]` вручную — `auth.WithAPIKeyHashSecret([]byte)` Option.
...
### Subcommand routing (v1.1.0)

`service.Boot` принимает variadic `BootOption`'ы. Самая полезная — регистрация subcommand-handler'ов через `BootSeed(name, fn)` или `WithSubcommand(name, fn)`. Когда первый command-line аргумент совпадает с зарегистрированным именем, kit dispatch'ит к этому handler'у вместо default fn:

```go
func main() {
    service.Boot(run,
        service.BootSeed("seed", seed),         // ./mybinary seed → seed(ctx)
        service.WithSubcommand("migrate", migrate),  // ./mybinary migrate → migrate(ctx)
        service.WithSubcommand("inspect", inspect),
    )
}
...
func seed(ctx context.Context) error {
    ...
    svc, err := service.New[AppCtx, Claims](ctx, cfg,
        ...
        service.WithoutCron(),    // не запускать cron в seed-режиме
        service.WithoutOpenAPI(), // /openapi.json не нужен для one-off CLI
    )
    if err != nil { return err }
    defer svc.Close()
    // insert demo data, print curl cheatsheet, return.
    return nil
}
```

| Invocation | Что запускается |
|---|---|
| `./mybinary` | `run` (default fn) |
| `./mybinary seed` | `seed` |
| `./mybinary migrate` | `migrate` |
| `./mybinary anything-else` | `run` (unknown subcommand → fall through) |

Subcommand fn'ы имеют ту же signature что и default fn (`func(ctx context.Context) error`) и получают тот же signal-aware ctx — SIGINT/SIGTERM во время long-running seed-операции prop'ируется чисто.

Каждый subcommand fn владеет своим `service.New` вызовом — kit намеренно НЕ передаёт pre-constructed Service в subcommand: тип-параметры Boot нести не может, и seed/migrate/inspect paths обычно хотят отличные от production'а option set'ы (`WithoutCron`, `WithoutOpenAPI`, но `WithMigrations`).

`BootSeed` — это named alias для `WithSubcommand` под common case (lit `BootSeed("seed", fn)` читается лучше чем `WithSubcommand("seed", fn)` в main.go). Identical semantics; используйте `WithSubcommand` для не-seed имен (migrate, inspect, schema-dump etc).
...
| `Env` | `ENV` | Пусто. `WithDevMode` использует как gate для mounting dev-only inspector'ов — non-`dev` значения отключают mounting даже когда option передан. |
| `CORSOrigins` | `CORS_ORIGINS` | Пусто. Comma-separated list of allowed origins. С v1.1.0 — если non-empty AND caller не вызвал `WithCORS`/`WithCORSConfig`, `service.New` auto-применяет `WithCORS(origins...)`. Whitespace trimmed; blank entries dropped. `*` среди origins → credentials off (CORS-спек). |
| `TLSCertFile` / `TLSKeyFile` | `TLS_CERT_FILE` / `TLS_KEY_FILE` | Пусто. Оба заданы → `Run()` слушает HTTPS (`fibermap.WithTLS` → `app.ListenTLS`). Пара all-or-nothing: один без второго → `Config.Validate` падает с `service_tls_config_incomplete`, тихого fallback на HTTP нет. Caller-опция `WithTLS` побеждает env-пару. |
...
| `APIKeyHashSecret` | `AUTH_APIKEY_HASH_SECRET` | пусто. Base64-encoded (любой flavour: std/url, padded/raw), ≥ 32 байт после decode. Required только когда сервис wire'ит API-key auth (`auth/fibermount.MountAPIKeyFactory`); pure-JWT сервисы оставляют пустым. Invalid base64 / короткое значение → `*errs.Error{Code: CodeAuthInvalidAPIKeyHashSecret}` на `service.New` time, не runtime panic. |
...
### Env-driven auto-enable для Sentry / OTel / CORS

С v1.0.1 `service.New` смотрит на стандартные env vars **после** того как применил caller's `With*` опции. Если что-то не подключено программно, но env vars присутствуют — оно подключается автоматически:

| Env | Триггерит | Source order |
|---|---|---|
| `SENTRY_DSN` (non-empty) | `WithSentry(dsn, SentryOptions{})` если `o.sentryDSN == ""` | — |
| `OTEL_EXPORTER_OTLP_ENDPOINT` либо `OTEL_EXPORTER_OTLP_TRACES_ENDPOINT` (non-empty) | `WithOtel(name, OtelOptions{})` если `o.otelServiceName == ""` | service.name: 1. `OTEL_SERVICE_NAME` 2. `cfg.Service.ServerGroup` 3. `cfg.Service.NodeName` 4. пропуск |
| `OTEL_SDK_DISABLED=true` | **kill switch** — OTel auto-enable skip'ается независимо от endpoint env | — |
| `CORS_ORIGINS=https://a.com,https://b.com` (с v1.1.0) | `WithCORS(origins...)` если caller не вызвал `WithCORS`/`WithCORSConfig`. Whitespace trimmed; blank entries dropped. AllowCredentials matches `WithCORS` контракт (`*` среди origins → credentials off). | — |

Caller-supplied `WithSentry` / `WithOtel` / `WithCORS` / `WithCORSConfig` всегда побеждает: env подхватывается только когда соответствующий слот остаётся пустым.

Если `cfg.Service.ServerGroup` и `NodeName` оба пустые, и `OTEL_SERVICE_NAME` не задан, OTel auto-enable пропускается (нет sensible default'а для service.name).

Sentry/OTel options (`sentrykit.Option...`, `otelkit.Option...`) при auto-enable остаются дефолтными — для tuning используйте `WithSentry`/`WithOtel` явно даже если DSN/endpoint берёте из env. То же про CORS: env-вариант применяет kit-defaulted `cors.Config` (AllowMethods REST set, AllowHeaders Authorization/Content-Type/X-Request-ID/X-Idempotency-Key, MaxAge 24h) — для custom-headers / `AllowOriginsFunc` / нестандартного MaxAge передавайте `WithCORSConfig` явно.
...
| `WithValidator(bind.Validator)` | Override default `validator.New(validator.WithRequiredStructEnabled())`. Для случая "kit defaults + один custom тэг" предпочтите [`WithExtraValidators`](#) ниже. |
| `WithExtraValidators(map[string]validator.Func)` | С v1.1.0. Регистрирует tag-name → validator.Func пары на kit-default `*validator.Validate`. Multiple calls accumulate, later wins на одинаковом теге. No-op для пустых map'ов. **Игнорируется, когда передан `WithValidator`** — кит не мутирует caller-shared instance. |
...
| `WithCORSConfig(cors.Config)` | Full-control CORS — `cfg` передаётся прямиком в `cors.New`. Оба варианта занимают выделенный слот перед bearer-слоем (после otelfiber); повторный вызов `WithCORS`/`WithCORSConfig` — last-write-wins. |
| `WithTLS(certFile, keyFile)` | App-native HTTPS: `Run()` зовёт `app.ListenTLS` вместо `Listen`. Для edge-деплоев без ingress-терминатора и локальной проверки `Secure`-cookie. Один файл без второго → ошибка `invalid_tls_config` на Run, не старт по HTTP. Env-эквивалент — `TLS_CERT_FILE`/`TLS_KEY_FILE`. mTLS/hot-reload — вне рамок (через `WithRunOptions`+`WithConfigureApp`). |
...
| `WithBodyLimit(bytes)` | Cap inbound request-body (Fiber возвращает 413 над limit'ом). 0 → Fiber-default (4 MiB). Теряет caller-supplied `fibermap.WithFiberConfig` через `WithRunOptions`. Note: `fibermap.ErrorHandler` устанавливается всегда — независимо от того, передан ли `WithBodyLimit` (с v1.0.1; до этого ErrorHandler был привязан к non-zero BodyLimit, и сервисы без явного limit'а получали fiber-дефолтный 500 plaintext на `*errs.Error`-возвратах). |
| `WithErrorHandler(fiber.ErrorHandler)` | С v1.1.0. Override default `fibermap.ErrorHandler(logger)`. Typical case — sentrykit 5xx auto-capture: `WithErrorHandler(sentrykit.WrapErrorHandler(fibermap.ErrorHandler(logger)))`. Nil = keep kit default. Composes cleanly с `WithBodyLimit` (оба применяются в `buildFiberConfig`). Caller-supplied `fibermap.WithFiberConfig` через `WithRunOptions` всё ещё wins (применяется позже в RunOption chain). |
| `WithDBOptions(opts...)` | Extra `db.Option`'ы, применяемые к kit-built `*db.DB`. `db.WithLogger` + `db.WithMetrics` уже подключены автоматически; reach for this, чтобы добавить `db.WithSlowQueryThreshold`, дополнительные `db.WithTracer` (audit / кастомные backends), `db.WithReadLagBudget`, `db.WithReplicaLagPolling` и т.д. **НЕ передавайте `db.WithMetrics(reg)` сюда без `WithoutAutoDBMetrics`** — двойная регистрация на одном registry panics. |
| `WithoutAutoDBMetrics()` | Отключает kit-дефолтный авто-wire `db.WithMetrics(s.metrics)`. Pair с `WithDBOptions(db.WithMetrics(otherReg))` когда нужно отдельный registry для db-метрик. |
...
| `WithCron(name, schedule, fn)` | Зарегистрировать recurring-job на config-time. 5-field cron-формат (override через `WithCronParser`). Auto-wrap'ит с `sentrykit.MonitorCron`, когда `WithSentry` подключён. Программный аналог `WithCronMap` — используйте для job'ов, чей schedule computed at startup или closure захватывает call-site state. |
...
| `WithCronMap()` | Декларативный cron через [`cronmap`](../cronmap/README.md) YAML (`crons.yaml`). Auto-loads из `Config.CronMap.Path` (или `DefaultCronMapPath` joined с ConfigsDir); auto-wires PGLocker когда DB сконфигурирован (YAML-flip на `singleton: true` без redeploy); auto-attach'ит `cronmap.WithSentry()` когда `WithSentry` подключён. Handlers регистрируются через `RegisterCronHandler(name, fn)` Option. Default-path missing — silent skip; explicit Path missing — `service_cronmap_yaml_not_found`. Singleton без DB — `service_cronmap_needs_db`. |
| `RegisterCronHandler(name, fn)` | Buffer'ит handler для apply'я на cronmap.Engine ПЕРЕД Build (паттерн `WithAPIMapRegistration`). Возвращает Option, composable с другими `With*` на call-site. |
| `WithCronMapEnv(map)` | Явные значения для `${VAR}` substitution в `crons.yaml`. Map консультируется до `os.LookupEnv`. |
