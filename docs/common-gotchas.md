...
| 5 | `auth.APIKeyFactory` panics with `api_key_missing_secret` on first request when `APIKeyHashSecret` empty | **Pending v1.1.0** — P0-2 thread the secret end-to-end (env `AUTH_APIKEY_HASH_SECRET` + `AuthConfig` field + `auth.WithAPIKeyHashSecret([]byte)` Option) |
| 6 | Default `defaultBindError` writes `{"error":"<full message>"}` plain-text, not `{code, message, details[]}` | **Pending v1.1.0** — P1-4 ships `fibermap.ErrsvalBindError[T]` as the recommended handler |
