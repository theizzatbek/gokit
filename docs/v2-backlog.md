...
forward), `WithInput` — generic. Documented ergonomic split,
но 5 функций для четырёх sources читается тяжело в `go doc`.
...
- **Triage:** `pending decision` — collapse был бы `v2-only`
  (signature change). Если оставляем — пометить как «kit
  convention» в `fibermap/doc.go`.
