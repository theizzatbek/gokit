...
- ✓ #2.10 `clients/redis.(*Client).Redis()` под cluster/sentinel — fail-fast panic с guiding-сообщением вместо silent nil-dereference; `clients/ratelimit.NewRedis` возвращает `*errs.Error{Code: CodeInvalidConfig}` вместо паники через `rc.Redis()`. → `fix/v1-redis-cluster-nil-trap` (`62b0da9`).
