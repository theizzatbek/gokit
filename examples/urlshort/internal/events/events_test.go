...
func TestPublisher_LinkVisited_LogsOnPublishError(t *testing.T) {
	...
	p.LinkVisited(context.Background(), LinkVisited{Code: "the-code"})
	out := buf.String()
	if !strings.Contains(out, "publish visited failed") {
